...
class hessian():
    ...
    def pruned_eigenvalues(self, maxIter=100, tol=1e-3, top_n=1):
        """
        compute the top_n eigenvalues using power iteration method for pruned networks
        maxIter: maximum iterations used to compute each single eigenvalue
        tol: the relative tolerance between two consecutive eigenvalue computations from power iteration
        top_n: top top_n eigenvalues will be computed
        """
        ...
        while computed_dim < top_n:
            ...
            # mask the random vector
            v = [v1 * mask1 for v1, mask1 in zip(v, masks)]
            
            v = normalization(v)  # normalize the vector

            for i in range(maxIter):
                v = orthnormal(v, eigenvectors)
                # mask the orthonormalized vector and normalize it
                v = [v1 * mask1 for v1, mask1 in zip(v, masks)]
                v = normalization(v)
                ...
                if self.full_dataset:
                    ...
                    tmp_eigenvalue = group_product(Hv, v).cpu().item()
                # mask the Hv
                Hv = [hvi * m for hvi, m in zip(Hv, masks)]
    ...
    def trace(self, maxIter=100, tol=1e-3):
        ...
        for i in range(maxIter):
            self.model.zero_grad()
            v = [
                torch.randint_like(p, high=2, device=device).to(self.device)
    ...
    def pruned_trace(self, maxIter=100, tol=1e-3):
        """
        compute the trace of hessian using Hutchinson's method for a pruned network
        maxIter: maximum iterations used to compute trace
        tol: the relative tolerance
        """
